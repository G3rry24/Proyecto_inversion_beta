...
#-------------------------------------------------------------------------------
st.set_page_config(page_title="Terminal Pro Interactiva", layout="wide")
st.title("📈 Terminal de Inversión Pro: Análisis 360°")
...
datos_radar = []
with st.expander("Estado actual del mercado", expanded=False):
    cols_por_fila = 4
    for i in range(0, len(lista_acciones), cols_por_fila):
        ...
        for j, t in enumerate(fila_tickers):
            try:
                ...
                if rsi_val < 35: cols[j].success(f"**{t}**\nRSI: {rsi_val:.1f} ✅")
                elif rsi_val > 65: cols[j].error(f"**{t}**\nRSI: {rsi_val:.1f} ⚠️")
                else: cols[j].info(f"**{t}**\nRSI: {rsi_val:.1f} ⚖️")
            except: cols[j].error(f"**{t}** Error")

if datos_radar:
    ganadora = min(datos_radar, key=lambda x: x['rsi'])
    st.warning(f"🏆 **Oportunidad Top:** {ganadora['ticker']} (RSI: {ganadora['rsi']:.1f})")

#-------------------------------------------------------------------------------
# 3. ANÁLISIS INTERACTIVO 3 PANELES
#-------------------------------------------------------------------------------
st.markdown("---")
...
if not datos.empty and len(datos) > 20:
    ...
    # CONFIGURACIÓN DE 3 SUBPLOTS
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, 
                       vertical_spacing=0.02, 
                       subplot_titles=(f'Velas e IA: {ticker_sel}', 'Volumen', 'Indicador RSI'), 
                       row_width=[0.2, 0.15, 0.65]) # Tamaños proporcionales

    # PANEL 1: Velas y Bandas
    ...
    fig.add_trace(go.Scatter(x=datos.index, y=datos['B_Sup'], line=dict(color='lightgray', width=1), name='B. Sup'), row=1, col=1)
    fig.add_trace(go.Scatter(x=datos.index, y=datos['B_Inf'], fill='tonexty', line=dict(color='lightgray', width=1), name='B. Inf'), row=1, col=1)
    ...
    # PANEL 2: Volumen
    fig.add_trace(go.Bar(x=datos.index, y=datos['Volume'], name='Volumen', marker_color='rgba(0,0,255,0.5)'), row=2, col=1)

    # PANEL 3: RSI Interactiva
    ...
    # Descarga
    st.download_button("📥 Descargar Reporte CSV", datos.to_csv().encode('utf-8'), 
                       f"analisis_{ticker_sel}_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")
